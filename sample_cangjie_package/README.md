...
This package includes a **browser CLI** in `web/`: a chat-style UI (scrollable command/response history, text input at the bottom). From the package root run `npm install ws`, then **`node web/cli_ws_server.js`**. Open **`http://localhost:8765/`** in your browser (the backend serves the HTML and WebSocket on port 8765; no separate frontend server needed). If you see **"unknown command '--' for cjpm run"**, your cjpm does not pass arguments to the run target; after `cjpm build`, set **`CLI_BIN`** to the path to the built executable (e.g. `CLI_BIN=$PWD/target/release/bin/main node web/cli_ws_server.js`) so the backend runs the CLI directly. Optional: to serve the frontend on port 3000 instead, run **`npx serve web`** from the **package root** (so the served root is the `web/` folder) and open `http://localhost:3000/`; if you run `npx serve .` then the page is at `http://localhost:3000/web/`. Type commands; output is shown in grey. Each message runs in its own process (refs and env are local to that message). Use semicolons in one message for multiple commands (e.g. `Student new Alice 1001 ; Student new Bob 1002`). Within one message you can use multiple assignments and `$NAME` (e.g. `SN1 = Student new Alice 1001 ; EV = Lesson new ; Lesson new ; $EV`); the line is processed segment-by-segment so later segments see earlier refs. Semantics: run command, see response, env vars (`NAME = command`, `$NAME`), refs (`ref:1`, `ref:2`, …). Type **`exit`** to close the session. If the session is idle for 10 minutes (or **`IDLE_TIMEOUT_MS`**), "session idle. exiting" is shown and the session closes. **Logs** are written to **`web/logs/cli_ws_server.log`**; use **`DEBUG_LOG=1 node web/cli_ws_server.js`** for extra debug detail (e.g. stdout/stderr chunk lengths).
