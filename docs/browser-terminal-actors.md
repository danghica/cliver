...
- **Backend (generated)**: Clive generates **`web/cli_ws_server.js`** (Node.js). It runs a WebSocket server; for each **message** (one line) it spawns a **one-off** Cangjie process with **`cjpm run --run-args="<line>"`**, collects stdout/stderr, and sends a single JSON `{ stdout, stderr }` to the client. **One process per message**: refs and env are session-local to that message (no persistence across messages). A single message can contain **semicolon-separated commands** (e.g. `Student new Alice 1001 ; Student new Bob 1002`); the driver splits by `;` and runs all segments in that one process, so refs (ref:1, ref:2, ...) are shared within the line. The user can type **`exit`** to close the session (backend sends `sessionClosed`, closes the WebSocket). If the session is **idle** (no input) for **`IDLE_TIMEOUT_MS`** (default 1 minute), the backend sends "session idle. exiting" and closes the connection. No PTY or long-lived process; no `node-pty` required.
...
- **`NAME = command`** — Runs the command and stores the last ref in `NAME`. Example: `STU = Student new Alice 1001`. (Refs and env are **local to that message**; they are not available in later messages because each message runs in a new process.)
- **`$NAME`** — Substitution **within the same message** (e.g. `STU = Student new Alice 1001 ; someCommand $STU`). Unset names are replaced with an empty string.
...
- One browser tab = one WebSocket connection; each message runs in its own process (refs and env local to that message). You can use `;` in one message to run multiple commands in the same process (e.g. `Student new Alice 1001 ; Student new Bob 1002`). If the session is idle for too long (default 1 minute), "session idle. exiting" is shown and the session closes.
